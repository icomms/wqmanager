...
from smsnotifications.models import SmsNotification, Manager
...
def index(request):
    ...
    notifications = SmsNotification.objects.all().order_by("-wqmauthority")
...
def check_notice_form(req):
    ...
    if SmsNotification.objects.filter( sampling_point = point, notification_type = notice_type  ):
