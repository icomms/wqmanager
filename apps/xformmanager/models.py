...
class FormDefModel(models.Model):
    ...
    def __unicode__(self):
        return self.form_display_name + " (" + self.form_name + ")"
...
class Metadata(models.Model):
    ...
    timestart = models.DateTimeField(_('Time start form'), default = datetime.utcnow)
    timeend= models.DateTimeField(_('Time end form'), default = datetime.utcnow)
    ...
    def save(self, target_namespace, **kwargs):
        ...
        # wqr phone number fix
        if self.username and self.username.isdigit() and self.username[0] != '0':
            self.username = '0' + self.username                
    ...
    def _log_bad_metadata(self, target_namespace):
        ...
        if missing_required_fields or null_required_fields:
            ...
            logging.error(""" Bad metadata submission to schema %s\nThe attachment is:\n%s\nThe errors are:\n%s%s""" % 
                          (target_namespace, self.attachment.display_string(), null_msg, missing_msg)) 
