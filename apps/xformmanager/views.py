...
def list_xforms(request, domain_name):
    context = {}    
    ...
    if not domain_name or not domain:
        return HttpResponse("Please specify a valid domain name, e.g. /xforms/list/&lt;domain_name&gt;")
    
    forms = {}
    db_forms = FormDefModel.objects.all().filter(domain=domain.id, active=True)    
    
    for db_form in db_forms:
        if not forms.has_key(db_form.form_display_name):
            forms[db_form.form_display_name] = db_form
        else:
            if db_form.version > forms[db_form.form_display_name].version:
                forms[db_form.form_display_name] = db_form

    context['forms'] = []
    
    for form in forms:
        context['forms'].append(forms[form])
           
    context['base_url'] = request.get_host() 
    
    return render_to_response(request, 'list_xforms.html', context)

def download_xform(request, formdef_id):
    context = {}               
    ...
    if not formdef_id or not xform:
        return HttpResponse("Please specify a valid form ID, e.g. /xforms/download/&lt;id&gt;")    
